---
title: "Health Sciences: Pre-Professional"
slug: HSPP
description: "Health Science: Pre-Professional is a two-year program providing
  a foundation in anatomy, physiology, medical terminology, and healthcare careers.
  Students learn patient care, professionalism, and communication skills while
  exploring labs in forensics, imaging, dental, cardiac, and more. Certifications
  include CPR/AED, CCMA, HIPAA, and OSHA 10, with clinical observations in two
  healthcare areas to prepare students for college and careers."
short_description: ''

kuula_id: 7DT9g

course:
  image_num: 8

teachers:
  - name: Gwen Barnett
    program: ''
    degree: ..
    career_start: ..
    certifications: ..
    email: gwen.barnett@evsck12.com
    phone: ..
    room: ..
    love_teaching: ..
    fun_fact: ..
  - name: Tonya Fleenor
    program: ''
    degree: ..
    career_start: 1994
    certifications: ''
    email: tonya.fleenor@evsck12.com
    phone: 930-214-4018
    room: HS15
    love_teaching: ..
    fun_fact: ..
  - name: Angie Pruitt
    program: ''
    degree: Master of Science in Nursing, University of Cincinnati
    career_start: 1986
    certifications: Clinical Nurse Specialist
    email: angela.pruitt@evsck12.com
    phone: 812-435-8809
    room: HS15
    love_teaching: Teaching in CTE is a rich environment where you can 
      impact a teens decision that may last a lifetime. To communicate and 
      prepare them for college and dealing with life is rewarding. The 
      confidence they gain sets them up for success and independence.
    fun_fact: I love to work with stained glass. I’ve also kept the 
      booksfor the Memorial Lacrosse JV team. I also played flute and 
      piccolo in the band.
yt_id: ''
---